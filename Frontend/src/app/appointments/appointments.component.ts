...
export class AppointmentsComponent implements OnInit {
  appointment: AppointmentDialogData = {
    topic: '',
    person: '',
    time: { hour: '', minute: '' },
    date: ''
  ...
  appointmentsList: dayAppointments[] = [
    {
      ...
      appointments: [
        { topic: 'Meeting', person: 'Kutay Tire', time: '13:33' },
        { topic: 'F.B.A Meeting', person: 'F.B.A.', time: '16:33' }
      ]
    },
    {
  ...
  deleteAppointment(dayAppointmentsIndex, appointmentIndex) {
    ...
    dialogRef.afterClosed().subscribe(deleteItem => {
      if (deleteItem) {
        this.appointmentsList[dayAppointmentsIndex]['appointments'].splice(
          appointmentIndex,
          1
        );
        if (
          this.appointmentsList[dayAppointmentsIndex]['appointments'].length ==
          0
        ) {
  ...
  deleteReceivedAppointmentRequest(dayAppointmentsIndex, appointmentIndex) {
    const dialogConfig = new MatDialogConfig();
    dialogConfig.data = {
      text: 'Are you sure to delete this appointment request?'
    };
    const dialogRef = this.dialog.open(
      ConfirmationDialogComponent,
      dialogConfig
    ...
    dialogRef.afterClosed().subscribe(deleteItem => {
      if (deleteItem) {
        this.appointmentsList2[dayAppointmentsIndex]['appointments'].splice(
          appointmentIndex,
          1
        );
        if (
          this.appointmentsList2[dayAppointmentsIndex]['appointments'].length ==
          0
        ) {
  ...
  deleteSentAppointmentRequest(dayAppointmentsIndex, appointmentIndex) {
    const dialogConfig = new MatDialogConfig();
    dialogConfig.data = {
      text: 'Are you sure to unsend this appointment request?'
    };
    const dialogRef = this.dialog.open(
      ConfirmationDialogComponent,
      dialogConfig
    ...
    dialogRef.afterClosed().subscribe(deleteItem => {
      if (deleteItem) {
        this.appointmentsList3[dayAppointmentsIndex]['appointments'].splice(
          appointmentIndex,
          1
        );
        if (
          this.appointmentsList3[dayAppointmentsIndex]['appointments'].length ==
          0
        ) {
